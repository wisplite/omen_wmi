...
/*
Added by Jonas
Brightness hotfix for OMEN 16t-wf100 laptops.
*/
#include <linux/backlight.h>
#include <linux/acpi.h>

static struct backlight_device *omen_bl_dev;

static int omen_discrete_bl_get_brightness(struct backlight_device *bd)
{
	return bd->props.brightness;
}

static int omen_discrete_bl_update_status(struct backlight_device *bd)
{
	int level = bd->props.brightness;
	acpi_status status;
	
	status = acpi_execute_simple_method(NULL, "\\_SB.PC00.PEG1.PEGP.DSP1._BCM", level);
...
static const struct backlight_ops omen_discrete_bl_ops = {
	.get_brightness = omen_discrete_bl_get_brightness,
	.update_status = omen_discrete_bl_update_status,
};

/* End of brightness hotfix */
			      ...
			      u32 attr, int channel, long val)
{
	...
	case hwmon_pwm:
		...
		case 0:
			...
			int r = hp_wmi_fan_speed_max_set(1);
			return r;
		case 2:
			/* 2 is automatic speed control, which is 0 for us */
			if (is_victus_s_thermal_profile()) {
				hp_wmi_get_fan_count_userdefine_trigger();
				int r = hp_wmi_fan_speed_max_reset();
				return r;
			} else {
				int r = hp_wmi_fan_speed_max_set(0);
				return r;
			}
		default:
...
static int __init hp_wmi_init(void)
{
	...
	*/
	...
	props.brightness = 50;
	omen_bl_dev = backlight_device_register("omen_backlight", NULL, NULL, &omen_discrete_bl_ops, &props);
	if (IS_ERR(omen_bl_dev)) {
		err = PTR_ERR(omen_bl_dev);
		pr_warn("hp-wmi: Failed to register backlight device: %d\n", err);
		omen_bl_dev = NULL;
	} else {
		pr_info("hp-wmi: Successfully registered omen_backlight device\n");
	}
	/* End of brightness hotfix */
...
static void __exit hp_wmi_exit(void)
{
	...
	/* Added by Jonas - Brightness hotfix cleanup */
	if (omen_bl_dev)
		backlight_device_unregister(omen_bl_dev);
}
